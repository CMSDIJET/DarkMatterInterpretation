...
else          : frame = C.cd(1).DrawFrame(0,0,3700,1700) 
...
else        : leg.SetHeader("#bf{Observed exclusion 90% CL}")
...
if DijetOnly:
    ...
    legd.SetTextColor(color["relic"]-1)
    legd.Draw("same")
elif not METXonly:
    g1.Draw("same")
...
if Mediator=="Vector" and DijetOnly:
    legw=TLatex(1670,120,"#Omega_{c} h^{2} #geq 0.12")
    ...
    legw.SetTextSize(0.04)
    legw2 = TLatex(1670,470,"#Omega_{c} h^{2} #geq 0.12")
    ...
    legw=TLatex(1750,160,"#Omega_{c} h^{2} #geq 0.12")
...
if Mediator=="Vector" and DijetOnly:
    leg1=TLatex(2850,1120,"#splitline{#bf{"+Mediator+" mediator}}{#bf{Dirac DM}}")
    leg4=TLatex(2850, 850,"#splitline{#it{g_{q} = 0.25}}{#it{g_{DM} = 1.0}}")
    leg4.SetTextFont(42)
    leg4.SetTextSize(0.040)
    leg4.Draw("same")
elif Mediator=="Axial" and DijetOnly:
    leg1=TLatex(2750,920,"#splitline{#bf{"+Mediator+"-vector mediator}}{#bf{Dirac DM}}")
    leg4=TLatex(2750, 650,"#splitline{#it{g_{q} = 0.25}}{#it{g_{DM} = 1.0}}")
#    leg4=TLatex(2750, 650,"#it{g_{q} = 0.25, g_{DM} = 1}")
    leg4.SetTextFont(42)
    leg4.SetTextSize(0.040)
    leg4.Draw("same")
elif Mediator=="Vector" and not METXonly:
...
if DijetOnly : 
    # CMS
    leg2=TLatex(75,1720,"#bf{CMS Preliminary}")
    leg2.SetTextFont(42)
    leg2.SetTextSize(0.045)
    # lumi
    leg3=TLatex(2350,1720,"27 fb^{-1} & 36 fb^{-1} (13 TeV)")
    leg3.SetTextFont(42)
    leg3.SetTextSize(0.045)
else         : 
    # CMS
    leg2=TLatex(100,1470,"#bf{CMS} #bf{Preliminary}")
    leg2.SetTextFont(42)
    leg2.SetTextSize(0.045)
    # lumi
...
tpad = TPad("p2" ,"",0.6900,0.1300,0.7400,0.300,0,0,0)
tpad.Draw()

tline1 = TLine(2760, 300, 2910, 300); 
tline1.SetLineColor(kAzure)
tline1.SetLineWidth(202)
tline1.SetLineStyle(kSolid)
tline1.Draw()

tline2 = TLine(2760, 130, 2910, 130);
tline2.SetLineColor(kAzure+1)
tline2.SetLineWidth(202)
tline2.SetLineStyle(kDotted)
tline2.Draw()
...
elif DijetOnly : C.SaveAs(Mediator+"_Dijet_DM_Mar2.pdf")
