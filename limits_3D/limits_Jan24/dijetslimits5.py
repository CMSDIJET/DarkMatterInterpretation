...
def main():
	...
	# read in the data: 
#	fin2 = ROOT.TFile("limits_freq_qq_calodijet2016_pfdijet2016_129.root");
        fin = ROOT.TFile("limits_freq_gg_qg_qq_pfdijet2016_95CL_moriondDataset_JECV1_35900pb.root");
        fin2 = ROOT.TFile("limits_freq_qq_calodijet2016.root");

#        obslo_129 = fin2.Get("obs_qq_calodijet2016");
#        obshi_129 = fin2.Get("obs_qq_pfdijet2016");
#        explo1_129 = fin2.Get("exp1sigma_qq_calodijet2016");
#        explo2_129 = fin2.Get("exp2sigma_qq_calodijet2016");
#        exphi1_129 = fin2.Get("exp1sigma_qq_pfdijet2016");
#        exphi2_129 = fin2.Get("exp2sigma_qq_pfdijet2016");

	obslo  = fin2.Get("obs_qq_calodijet2016");
	explo1 = fin2.Get("exp1sigma_qq_calodijet2016");
	explo2 = fin2.Get("exp2sigma_qq_calodijet2016");
...
#        obs_gq_129 = getobs(obslo_129,obshi_129,xs);
#	 exp_gq_129, exp1s_gq_129 = getexp(explo1_129,exphi1_129, xs);
#        exp_gq_tmp2_129, exp2s_gq_129 = getexp(explo2_129,exphi2_129,xs);

	obs_gq = getobs(obslo,obshi,xs);
	exp_gq, exp1s_gq = getexp(explo1,exphi1,xs);
	exp_gq_tmp2, exp2s_gq = getexp(explo2,exphi2,xs);

#	exp_gq_129.SetLineStyle(2);
#        exp_gq_129.SetLineColor(ROOT.kRed);
#	obs_gq_129.SetLineColor(ROOT.kRed);
...
#	DMMInffunc.SetLineWidth(2);
	...
	txta = ROOT.TLatex(0.19,0.93,"CMS Preliminary"); txta.SetNDC();
	txtc = ROOT.TLatex(0.68,0.96,"36 fb^{-1} (13 TeV)");
	txtc.SetNDC(); txtc.SetTextFont(42); txtc.SetTextSize(0.04);

	txtd = ROOT.TLatex(0.22,0.82,"#bf{95% CL upper limits}");
	txtd.SetNDC(); txtd.SetTextFont(42); txtd.SetTextSize(0.04);
	
	leg = ROOT.TLegend(0.20,0.67,0.55,0.80);
	leg.SetFillStyle(0);
	leg.SetFillColor(0);    
	leg.SetBorderSize(0);
	leg.SetTextFont(42); 
	# leg.AddEntry(obs_gq,"95% CL upper limits","")
	leg.AddEntry(obs_gq,"Dijet Observed_36^-1","l")
	leg.AddEntry(exp_gq,"Expected_36^-1","l")
#        leg.AddEntry(obs_gq_129,"Observed_12.9^-1","l")
#        leg.AddEntry(exp_gq_129,"Expected_12.9^-1","l")
